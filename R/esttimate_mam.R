mam_constraint_violations <- function(x, etsenv)
{
...
prepare_inputs_mam_ad <- function(spec, solver = "nloptr")
{
    ...
    if (solver %in% c("nloptr")) {
        ...
        {
            llh <- etsenv$llh
            if (mam_constraint_violations(x, etsenv)) {
                llh <- llh + 0.2 * abs(llh)
            } else {
                names(x) <- etsenv$parameter_names
                llh <- fun$fn(x)
    ...
    names(lower) <- rnames
    names(upper) <- rnames
    out <- list(data = list(model = "mam", y = y, x = x, good = good, fmat = fmat, 
...
estimate_mam_ad <- function(object, solver = "nloptr", control = list(trace = 1), ...)
{
    spec_list <- prepare_inputs_mam_ad(object, solver = solver)
    other_opts <- list(...)
