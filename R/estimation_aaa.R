aaa_constraint_violations <- function(x, etsenv)
{
    ...
    if (parmatrix["gamma","estimate"] == 1) {
        if (parmatrix["gamma","init"] > (1 - parmatrix["alpha","init"])) {
            v <- TRUE
            return(v)
        }
    }
...
prepare_inputs_aaa_ad <- function(spec, solver = "nloptr")
{
    ...
    if (solver %in% c("nloptr")) {
        ...
        if (spec$model$parmatrix["beta","estimate"] == 1 & spec$model$parmatrix["gamma","estimate"] == 1) {
            ineq_fun <- function(x, fun, etsenv)
            {
                ...
                ineq <- rbind(pmatrix["beta","init"] +  1e-4 - pmatrix["alpha","init"],
                              pmatrix["gamma","init"] + 1e-4 + pmatrix["alpha","init"]  - 1 )
                return(ineq)
            }
            ineq_jac <- function(x, fun, etsenv)
            {
                jac <- matrix(0, ncol = length(etsenv$estimation_names), nrow = 2)
                colnames(jac) <- etsenv$estimation_names
                jac[1,c("alpha","beta")]  <- c(-1, 1)
                jac[2,c("alpha","gamma")] <- c( 1, 1)
                return(jac)
            }
        } else if (spec$model$parmatrix["beta","estimate"] == 1 & spec$model$parmatrix["gamma","estimate"] == 0) {
            ineq_fun <- function(x, fun, etsenv)
            {
                pmatrix <- etsenv$parmatrix
                names(x) <- etsenv$estimation_names
                pmatrix[etsenv$estimation_names,"init"] <- x
                ineq <- matrix(pmatrix["beta","init"] + 1e-4 - pmatrix["alpha","init"], 1, 1)
                return(ineq)
            }
            ineq_jac <- function(x, fun, etsenv)
            {
        ...
        } else if (spec$model$parmatrix["beta","estimate"] == 0 & spec$model$parmatrix["gamma","estimate"] == 1) {
            ineq_fun <- function(x, fun, etsenv)
            {
                pmatrix <- etsenv$parmatrix
                names(x) <- etsenv$estimation_names
                pmatrix[etsenv$estimation_names,"init"] <- x
                ineq <- matrix(pmatrix["gamma","init"]  + 1e-4 + pmatrix["alpha","init"]  - 1, 1, 1)
                return(ineq)
            }
            ineq_jac <- function(x, fun, etsenv)
            {
    ...
    names(lower) <- rnames
    names(upper) <- rnames
    out <- list(data = list(model = "aaa", y = y, x = x, good = good, fmat = fmat, 
...
estimate_aaa_ad <- function(object, solver = "nloptr", control = list(trace = 1), ...)
{
    spec_list <- prepare_inputs_aaa_ad(object, solver = solver)
    other_opts <- list(...)
